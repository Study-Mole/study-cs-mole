...
일반적으로 테스트는 다음과 같이 일정한 패턴으로 작성한다.
...
때로는 코드가 **시간이 걸리는 작업**을 할 수 있다. 예를 들어 서버에서 데이터를 가져오는 코드를 테스트 하는경우
...
서버에서 데이터를 받아오는 코드 대신, 가짜 데이터를 써서 테스트할 수 있다.
