...
JWT 토큰은 탈취의 위험이 있기 때문에, 유효기간이 서로 다른 Access Token과 Refresh Token을 두어 피해를 최소화 합니다. Access Token은 접근에 관여하는 토큰으로 유효기간이 짧고, Refresh Token은 재발급에만 관여하는 토큰으로 유효기간이 깁니다.
