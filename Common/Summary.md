...
사용자가 페이지를 떠날 때, 진행 중인 HTTP 요청을 끝까지 보장하지 않습니다. 요청을 끝까지 살려두기 위해서 `fetch() + keepalive 옵션`을 사용하여 언마운트 되어도 요청이 계속 진행되도록 하는 방법, `Navigator.sendBeacon()`을 사용하여 낮은 우선순위이지만 끝까지 요청을 처리하는 방법, `<a> 태그에 ping을 지정` 하여 링크 이동 시 작은 post 요청을 보내는 방법 등이 있습니다.