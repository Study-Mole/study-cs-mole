...
DOMContentLoaded는 HTML 문서의 DOM 트리가 완전히 구성되었을 때 발생하는 이벤트입니다. 이는 브라우저가 HTML을 파싱하고 DOM 트리를 생성한 시점을 의미하며, 외부 리소스인 이미지나 스타일시트가 완전히 로드되지 않았더라도 발생합니다.

이 이벤트는 주로 DOM 요소에 접근하거나 조작해야 하는 JavaScript 코드를 실행하기에 적합한 시점을 알려줍니다. 예를 들어 document.addEventListener('DOMContentLoaded', callback) 형태로 사용합니다.
window.onload와의 차이점은, DOMContentLoaded는 DOM 구조만 준비되면 발생하지만, load 이벤트는 모든 외부 리소스까지 로드된 후에 발생한다는 점입니다. 그래서 일반적으로 사용자 경험 향상을 위해 DOMContentLoaded를 사용하는 경우가 많습니다.

> **HTML에서의 스크립트 로딩 순서에 대해서 설명해주세요.**

HTML에서 스크립트 로딩은 기본적으로 순차적으로 이루어집니다. 브라우저는 HTML을 파싱하다가 script 태그를 만나면 파싱을 중단하고 스크립트를 로드하고 실행한 후 다시 파싱을 재개합니다.

이러한 동작을 제어하기 위해 script 태그에 async와 defer 속성을 사용할 수 있습니다:
일반 script 태그는 HTML 파싱을 중단시키므로 페이지 로딩 속도에 영향을 줄 수 있습니다.
async 속성이 있는 스크립트는 HTML 파싱과 병렬로 다운로드되며, 다운로드가 완료되면 즉시 실행됩니다. 이때 HTML 파싱이 잠시 중단됩니다. 실행 순서가 보장되지 않아 독립적인 스크립트에 적합합니다.
defer 속성이 있는 스크립트도 병렬로 다운로드되지만, HTML 파싱이 완료된 후 DOMContentLoaded 이벤트 직전에 순서대로 실행됩니다. DOM에 의존적인 스크립트에 적합합니다.

실무에서는 페이지 렌더링 속도를 높이기 위해 필수적이지 않은 스크립트는 defer나 async를 사용하고, 특히 DOM 조작이 필요한 스크립트는 defer를 사용하는 것이 좋은 방법입니다.
